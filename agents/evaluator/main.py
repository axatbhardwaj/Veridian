...
async def generate_keywords_endpoint(req: ContentRequest):
    # Use the new evaluate function internally
    evaluate_req = EvaluateRequest(title=req.title, markdown=req.content)
    result = evaluate(evaluate_req)
    
    # Convert response format
    return ContentResponse(
        keywords=result.keywords,
        price_per_call=result.price_usdc_cents / 100.0  # Convert cents to dollars
    )


@app.post("/match_topic", response_model=TopicResponse)
async def match_topic_endpoint(req: TopicRequest):
    # Generate keywords for the topic using heuristic method
    topic_keywords = _extract_heuristic_keywords(req.topic)
    ...
    try:
        ...
        logging.error(f"Failed to fetch content hashes: {e}")
...
async def health_check():
    return {"status": "ok", "message": "Evaluator service is running", "gemini": bool(GEMINI_API_KEY)}
